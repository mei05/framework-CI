...
class Detail_model extends CI_Model
{
	...
	public function get()
	{
		...
		$this->db->join('menu p', 'k.id_menu = p.id');
		$query = $this->db->get();
	...
	public function getById($id)
	{
		...
		$this->db->select('d.*,r.nama as nama, b.nama as menu');
		$this->db->from('detail_penjualan d');
		$this->db->join('user r', 'd.id_user = r.id');
		$this->db->join('menu b', 'd.id_menu = b.id');
	...
	public function getByUser($id)
	{
		...
		$this->db->select('d.*,p.nama as nama');
		$this->db->from('detail_penjualan d');
		$this->db->join('menu p', 'd.id_menu = p.id');
	...
	function charts()
    {
        $this->db->select('d.*, b.nama as menu, sum(d.jumlah) as jum');
        $this->db->from('detail_penjualan d');
        $this->db->join('menu b', 'd.id_menu = b.id');
