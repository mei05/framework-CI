...
class Profil extends CI_Controller
{
	...
	function tentang()
	{
		...
		$data['jlh'] = $this->Keranjang_model->jumlah();
		$this->load->view("layout/header", $data);
		$this->load->view("user/vw_tentang", $data);
		$this->load->view("layout/footer", $data);
	}
	...
	{
		$jumlah_beli = count($this->input->post('menu'));
		$data_p = [
		...
		for ($i = 0; $i < $jumlah_beli; $i++) {
			array_push($data_detail, ['id_menu' => $this->input->post('menu')[$i]]);
		...
		if ($this->Penjualan_model->insert($data_p, $upload_image) && $this->Detail_model->insert($data_detail)) {
			//for ($i = 0; $i < $jumlah_beli; $i++) {
			//	$this->Menu_model->min_stok($data_detail[$i]['jumlah'], $data_detail[$i]['id_menu']) or die('gagal min stok');
			//}
